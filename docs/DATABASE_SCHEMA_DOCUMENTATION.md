...
VACUUM ANALYZE daily_pm25;
VACUUM ANALYZE population;

-- Update table statistics
ANALYZE yearly_pm25_summary;
ANALYZE monthly_pm25_summary;
ANALYZE seasonal_pm25_summary;
```

**Index Maintenance**:
```sql
-- Rebuild indexes if needed
REINDEX INDEX CONCURRENTLY idx_daily_pm25_fips_date;
REINDEX INDEX CONCURRENTLY idx_yearly_pm25_fips_year;
```
...
**1. Summary Table Usage**:
```sql
-- Use summary tables instead of aggregating raw data
SELECT * FROM yearly_pm25_summary 
WHERE fips = '06001' AND year = 2020;

-- Instead of:
SELECT AVG(total), AVG(fire), AVG(nonfire) 
FROM daily_pm25 
WHERE fips = '06001' AND EXTRACT(YEAR FROM date) = 2020;
```

**2. Spatial Indexing**:
```sql
-- PostGIS spatial indexes for geometry queries
CREATE INDEX idx_counties_geometry_gist ON counties USING GIST(ST_GeomFromGeoJSON(geometry));
```

**3. Partitioning Strategy**:
```sql
-- Partition daily_pm25 by year for better performance
CREATE TABLE daily_pm25_2020 PARTITION OF daily_pm25
...
BACKUP_DIR="/backups/pm25_dashboard"

# Create backup directory
mkdir -p $BACKUP_DIR

# Full database backup
pg_dump -h localhost -U postgres -d pm25_dashboard \
    --format=custom --file=$BACKUP_DIR/full_backup_$DATE.dump

# Backup only schema
pg_dump -h localhost -U postgres -d pm25_dashboard \
    --schema-only --format=plain --file=$BACKUP_DIR/schema_$DATE.sql

# Backup only data (without schema)
pg_dump -h localhost -U postgres -d pm25_dashboard \
    --data-only --format=custom --file=$BACKUP_DIR/data_$DATE.dump
...
```bash
# Restore from backup
pg_restore -h localhost -U postgres -d pm25_dashboard \
    --clean --if-exists full_backup_20240115_143000.dump
```
...
GRANT CONNECT ON DATABASE pm25_dashboard TO api_user;
GRANT USAGE ON SCHEMA public TO api_user;
GRANT SELECT ON ALL TABLES IN SCHEMA public TO api_user;

-- Create application user for data loading
CREATE USER app_user WITH PASSWORD 'secure_password';
GRANT ALL PRIVILEGES ON DATABASE pm25_dashboard TO app_user;
...
ALTER TABLE daily_pm25 ADD CONSTRAINT check_pm25_positive 
    CHECK (total >= 0 AND fire >= 0 AND nonfire >= 0);

ALTER TABLE population ADD CONSTRAINT check_population_positive 
    CHECK (population >= 0);

ALTER TABLE daily_pm25 ADD CONSTRAINT check_date_range 
    CHECK (date >= '2006-01-01' AND date <= '2023-12-31');
```
...
SELECT 
    ...
    correlation
FROM pg_stats 
WHERE tablename IN ('daily_pm25', 'counties', 'population')
...
This comprehensive database schema documentation provides developers and database administrators with all necessary information to understand, maintain, and optimize the PM₂.₅ Wildfire Impact Map database. For additional technical details or implementation questions, refer to the main documentation or contact the development team. 